...
void setup() {   //действия, выплняемые однокрано при запуске микроконроллера
  ...
  pinMode(chrPin, INPUT); //назначение входа для контроля факта работы внешнего блока питания
...
LedOut2 = LedOut2 + (leds[i]*reg[i-8]); //перемножение состояния светодиода на маску
    ...
    shiftOut(SER, CLK, LSBFIRST, LedOut1);  //выводим состояние светодиодов для первого сдвигового регистра
    shiftOut(SER, CLK, LSBFIRST, LedOut2); //выводим состояние светодиодов для второго сдвигового регистра
